...
class AlimentarBase:
    ...
    def add(self, file_path:str):
        ...
        with app.books.open(self.file_path) as wb:
            ...
            df_temp = pd.DataFrame(sheet.range('A1').expand('table').value)
            ...
            df_merge['Divisão'] = [divisao_formula.replace("B2", f"B{n+1}") for n in range(1, (len(df_merge)+1))]
            
            conta_formula:str = sheet.range('E2').formula
            df_merge['Conta 1'] = [conta_formula.replace("D2", f"D{n+1}") for n in range(1, (len(df_merge)+1))]            
